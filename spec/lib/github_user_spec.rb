...
RSpec.describe GithubUser do
  ...
  describe '::new' do
    it 'should store login, name and repositories' do
      user = GithubUser.new login: login, name: name, repositories: repositories
  ...
  describe '::from' do
    ...
    it 'should create a new user with login, name and repositories' do
      ...
      expect(GithubUser).to have_received(:new).with(login: login, name: name, repositories: repositories)
    end
  end
  ...
  describe '#==' do
    ...
    it 'should return false if two objects have different login' do
      user1 = GithubUser.new login: login, name: name, repositories: repositories
      user2 = GithubUser.new login: Faker::Superhero.name, name: name, repositories: repositories
      expect(user1 == user2).to be false
    end

    it 'should return false if two objects have different name' do
      user1 = GithubUser.new login: login, name: name, repositories: repositories
      user2 = GithubUser.new login: login, name: Faker::Superhero.name, repositories: repositories
      expect(user1 == user2).to be false
    end
